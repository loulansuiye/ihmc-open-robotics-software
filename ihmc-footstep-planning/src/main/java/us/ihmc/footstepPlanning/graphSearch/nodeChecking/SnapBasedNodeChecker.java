...
public class SnapBasedNodeChecker extends FootstepNodeChecker
{
   ...
   public boolean isNodeValid(FootstepNode node, FootstepNode previousNode)
   {
      ...
      if (snapTransform.containsNaN())
      {
         ...
         notifyPlannerListenerThatNodeIsRejected(node, previousNode, BipedalFootstepPlannerNodeRejectionReason.COULD_NOT_SNAP);
         return false;
      }
      ...
      if (!footholdAfterSnap.isEmpty() && area < parameters.getMinimumFootholdPercent() * footArea)
      {
         ...
         notifyPlannerListenerThatNodeIsRejected(node, previousNode, BipedalFootstepPlannerNodeRejectionReason.NOT_ENOUGH_AREA);
         return false;
      }
      ...
      if (heightChange > parameters.getMaximumStepZ())
      {
         ...
         notifyPlannerListenerThatNodeIsRejected(node, previousNode, BipedalFootstepPlannerNodeRejectionReason.STEP_TOO_HIGH_OR_LOW);
         return false;
      }
                                                       ...
                                                       parameters.getBodyGroundClearance()))
      {
         ...
         notifyPlannerListenerThatNodeIsRejected(node, previousNode, BipedalFootstepPlannerNodeRejectionReason.OBSTACLE_BLOCKING_BODY);
         return false;
      }
   ...
   private void notifyPlannerListenerThatNodeIsRejected(FootstepNode node, FootstepNode parentNode, BipedalFootstepPlannerNodeRejectionReason rejectionReason)
   {
      for (BipedalFootstepPlannerListener listener : listeners)
         listener.nodeUnderConsiderationWasRejected(node, parentNode, rejectionReason);
   }
}
